...
export async function createAttachment(
  ...
  // Inherit owner from the task (shared trees → owner is the original sharer).
  ...
  const storageKey = `attachments/${ownerId}/${parsed.taskId}/${id}-${safeName}`;
 ...
 * base64-encode large bodies into tool arguments.
 */
export async function addAttachmentFromUrl(
  ...
  return createAttachment(ctx, { taskId, filename, mimeType, body: buf, source });
}
...
export async function deleteAttachment(ctx: Context, id: string): Promise<void> {
  ...
  const row = rows[0];
  if (!row) throw new NotFoundError(id);
  // Deletion is owner-only — same rule as task delete.
