...
export const TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    description: [
      ...
      'attachments[] — files attached to the root task (filename, mimeType, sizeBytes, storage). Download URL: /api/files/<id> — send the same `Authorization: Bearer <GETSHIT_TOKEN>` header you use for MCP (agents have no browser session, so an unauthenticated GET returns 401).',
  ...
  },
  {
    ...
    description: [
      ...
      'Source is auto-tagged "agent". Capped by GETSHIT_MAX_UPLOAD_MB (default 25). After upload, the file is available at /api/files/<id> (send `Authorization: Bearer <GETSHIT_TOKEN>` to download it) and appears in get_task(taskId).attachments or get_note(noteId).attachments.',
      'Prefer this over base64-in-args for any file beyond a few KB — including local files, esp. video.',
      'LOCAL FILE (no public URL, e.g. a video on disk): do NOT base64 it. Serve its folder over http locally (`python3 -m http.server 8000`) and pass a `http://localhost:8000/<file>` URL here. When the MCP runs as a local stdio server it can fetch localhost, so the bytes go server-side straight to storage at full size (bounded only by GETSHIT_MAX_UPLOAD_MB), never through the agent context.',
  ...
  },
  {
    ...
    description: [
      ...
      'Prefer attach_file_from_url for large files (esp. video), whether the source is a public URL or a LOCAL file. Two independent limits bite here: (1) on a hosted/serverless endpoint the request body is capped by the platform (~4.5MB on Vercel), and (2) regardless of server, the base64 string is passed as a tool argument through the agent, so anything more than a few MB blows the model output/context limit before the server ever sees it. URL-fetch avoids both and is bounded only by GETSHIT_MAX_UPLOAD_MB. For a LOCAL file with no public URL, serve it over http locally (`python3 -m http.server`) and give attach_file_from_url a `http://localhost:PORT/<file>` URL — a local stdio MCP can fetch localhost.',
      'Source is auto-tagged "agent". After upload the file is at /api/files/<id> (send `Authorization: Bearer <GETSHIT_TOKEN>` to download it) and appears in get_task(taskId).attachments or get_note(noteId).attachments; the first image (else video) becomes the calendar thumbnail.',
    ].join(' '),
