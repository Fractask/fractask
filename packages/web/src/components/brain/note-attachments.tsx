...
import {
  addNoteAttachmentByUrlAction,
  deleteNoteAttachmentAction,
  revalidateBrainNoteAction,
...
/**
 * Mirrors `TaskAttachments` shape (drop zone, gallery, file list, URL mode)
 * but targets brain notes. A separate component (rather than widening the
 * task one) keeps the working task flow untouched while we iterate on the
 * note UX.
 */
export function NoteAttachments({
  ...
  const uploadFiles = (files: FileList | File[]) => {
    ...
    start(async () => {
      ...
      await revalidateBrainNoteAction(noteId);
    });
  };
  ...
  const submitUrl = () => {
    ...
    setError(null);
    start(async () => {
      const r = await addNoteAttachmentByUrlAction(noteId, trimmed);
  ...
  const remove = (id: string) => {
    start(async () => {
      const r = await deleteNoteAttachmentAction(id, noteId);
      if (!r.ok) setError(r.error);
      else await revalidateBrainNoteAction(noteId);
    });
  };
      ...
      >
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            disabled={pending}
            className="inline-flex items-center gap-1.5 rounded border border-(--color-border) bg-(--color-bg) px-2 py-1 text-xs hover:border-(--color-accent) disabled:opacity-50 cursor-pointer"
          >
            <Upload size={12} />
            Choose files
          </button>
          ...
          <button
            ...
            className="text-xs text-(--color-muted) hover:text-(--color-fg) cursor-pointer"
          >
        ...
        {urlMode && (
          <div className="mt-2 flex items-center gap-2">
            ...
            <button
              ...
              className="rounded bg-(--color-accent) px-2 py-1 text-xs font-medium text-white disabled:opacity-50 cursor-pointer"
            >
              {pending ? 'Fetching…' : 'Attach'}
            </button>
          </div>
        )}
        {pending && !urlMode && <p className="mt-2 text-xs text-(--color-muted)">Uploading…</p>}
